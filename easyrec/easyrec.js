 ...
 *  (c) 2008-2009 Research Studios Austria Forschungsgesellschaft mbH
 *
...
// This functions call the recommender to retrieve a list
...
function drawRecommendationListWithPictures(json){drawRecommendationListWithPicturesToDiv(json,"recommenderDiv");}
...
var easyrec_carusel_jqueryJs = false;
var easyrec_carusel_jcarouselJs = false;
function drawRecommendationCarusel(json){
    drawRecommendationCaruselToDiv(json, "recommenderDiv");
}
function drawRecommendationCaruselToDiv(json, recommenderDiv){
    if(!easyrec_carusel_jqueryJs || !easyrec_carusel_jcarouselJs){
        easyrec_includeJavascript(easyrecApiUrl +"../../../js/api/carusel/jquery-1.4.2.min.js",function(){easyrec_carusel_jqueryJs =true;drawRecommendationCaruselControlFunction(json,recommenderDiv)});
        easyrec_includeJavascript(easyrecApiUrl +"../../../js/api/carusel/jquery.jcarousel.min.js",function(){easyrec_carusel_jcarouselJs =true;drawRecommendationCaruselControlFunction(json,recommenderDiv)});
        easyrec_includeCSS(easyrecApiUrl +"../../../js/api/carusel/carousel.css");
    }else{
        drawRecommendationCaruselToDivWithJsReady(json, recommenderDiv);
    }
}

function drawRecommendationCaruselControlFunction(json, recommenderDiv){
	if(easyrec_carusel_jqueryJs && easyrec_carusel_jcarouselJs){
		drawRecommendationCaruselToDivWithJsReady(json, recommenderDiv);
	}
}
function drawRecommendationCaruselToDivWithJsReady(json, recommenderDiv){
 	if("undefined"==typeof(json.error) ) { // if no error show recommendations
                ...
                // display recommendations in the DIV layer 'recommendation'
                if (items.length>0) {
					var listString ="<ul id='recCarousel_"+recommenderDiv+"'  class='jcarousel-skin-easyrec'>";

                    for (x=0;x<15 && x <items.length;x++) {
                          listString +=
                              "<li style='text-align:center;'>"+
                              "<a class='jcarousel-skin-easyrec-link' href='"               + items[x].url + "'>" +
                              "<img width='150' height='150' style='border:0px;' alt='" + items[x].description + "'"+
                              "     src='"+ items[x].imageUrl + "'/><br/>"
                               + items[x].description +
                              "</a>" +
                              "</li>";
                    ...
                    jQuery(document).ready(function() {
					    jQuery('#recCarousel_'+recommenderDiv).jcarousel({initCallback:drawRecommendationCaruselToDiv_loadButtonPics});
					});
                }
         }
}

function drawRecommendationCaruselToDiv_loadButtonPics(){
                     $(".jcarousel-next")[0].style.backgroundImage= "url('"+easyrecApiUrl+"../../../js/api/carusel/next-horizontal.png')";
                    $(".jcarousel-prev")[0].style.backgroundImage= "url('"+easyrecApiUrl+"../../../js/api/carusel/prev-horizontal.png')";
...
//
function easyrec_includeJavascript (jsAdr,onLoadFunction) {
    var newJs = document.createElement('script');
    newJs.type = 'text/javascript';
    newJs.src = jsAdr;
    newJs.onload= onLoadFunction;
    document.getElementsByTagName('head')[0].appendChild(newJs);
}

function easyrec_includeCSS(cssAdr) {
